...
class ModelArguments:
    ...
    num_primitive: int = field(
        default=768,
        metadata={
            "help": (
                "The number of primitives to be considered, 768 and 1024 are the default for base and large models."
            )
        },
    )
    ...
    only_eng_vocab: bool = field(
        default=False,
        metadata={"help": "will use the English vocabulary as the source of the similarity calculation."},
    )
    use_initialization: bool = field(
        default=True,
        metadata={"help": "whether OFA initialization is used for the embedding layer."},
    )

    random_initialization: bool = field(
        default=False,
        metadata={"help": "whether random initialization is used for the embedding layer."},
    )
...
def main():
    ...
    if os.path.isdir(training_args.output_dir) and training_args.do_train and not training_args.overwrite_output_dir:
        ...
        logger.info(f"Will load from {last_checkpoint}!")
    ...
    if model_args.model_name_or_path:
        ...
        if model_args.use_initialization:
            ...
            if last_checkpoint is not None:
                ...
                logger.info(f"Loading model from {last_checkpoint}/pytorch_model.bin")
                state_dict = torch.load(last_checkpoint + '/pytorch_model.bin')
